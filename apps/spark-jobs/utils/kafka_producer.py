...
def get_kafka_producer() -> Producer:
    ...
    try:
        ...
        config = {
            ...
            # "buffer.memory": 134217728,  # Java Kafka 전용 설정 - confluent-kafka에서 미지원
            "queue.buffering.max.kbytes": 131072,  # 128MB (librdkafka 설정)
            # 처리량 최적화
        ...
        logger.info(
            f"Producer config: batch.size={config['batch.size']//1024}KB, queue.buffering.max=128MB"
        )
        return producer
